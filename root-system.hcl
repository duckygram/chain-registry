locals {
  ...
  system_name = basename(get_terragrunt_dir())
  source_path = (startswith(local.env_cfg.template.url, "git::") ?
    "${local.env_cfg.template.url}/${local.system_name}/terraform?ref=${local.env_cfg.template.ref}" :
    "${get_terragrunt_dir()}/../${local.env_cfg.template.url}/${local.system_name}/terraform"
  )

  global_name_prefix = "devlab"
}
...
remote_state {
  ...
  config = {
    ...
    dynamodb_table = "${local.backend_name}-tf-state-lock"
  }
  generate = {
    path      = "backend.tf"
...
generate "provider-eks-admin" {
  contents = templatefile(find_in_parent_folders("provider-aws.tftpl"), {
    args = {
      alias    = "eks_admin"
      region   = local.region
      iam_role = "arn:aws:iam::${local.account_id}:role/${local.cluster_name}-ClusterAdminRole"
      tags = {
        managed-with = "terraform"
        project      = local.env_cfg.project_name
        environment  = local.env_cfg.environment
